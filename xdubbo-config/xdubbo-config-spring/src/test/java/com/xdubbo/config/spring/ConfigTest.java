 ...
 */
package com.xdubbo.config.spring;
...
import com.xdubbo.config.*;
import com.xdubbo.config.spring.action.DemoActionByAnnotation;
import com.xdubbo.config.spring.action.DemoActionBySetter;
import com.xdubbo.config.spring.annotation.consumer.AnnotationAction;
import com.xdubbo.config.spring.api.DemoService;
import com.xdubbo.config.spring.api.HelloService;
import com.xdubbo.config.spring.filter.MockFilter;
import com.xdubbo.config.spring.impl.DemoServiceImpl;
import com.xdubbo.config.spring.impl.HelloServiceImpl;
import com.xdubbo.config.spring.registry.MockRegistry;
import com.xdubbo.config.spring.registry.MockRegistryFactory;
...
import com.xdubbo.rpc.service.GenericService;
...
public class ConfigTest {
    ...
    public void testToString() {
        ...
        assertTrue(str.contains(" interface=\"com.xdubbo.config.spring.api.DemoService\" "));
    ...
    public void testMultiProtocolRegister() {
        ...
        ctx.start();
        try {
            List<URL> urls = registryService.getRegistered().get("com.xdubbo.config.spring.api.DemoService");
            assertNotNull(urls);
            assertEquals(1, urls.size());
            assertEquals("dubbo://" + NetUtils.getLocalHost() + ":20824/com.xdubbo.config.spring.api.DemoService", urls.get(0).toIdentityString());
        } finally {
            ctx.stop();
            ctx.close();
            exporter.unexport();
    ...
    public void testMultiRegistry() {
        ...
        ctx.start();
        try {
            List<URL> urls1 = registryService1.getRegistered().get("com.xdubbo.config.spring.api.DemoService");
            assertNull(urls1);
            List<URL> urls2 = registryService2.getRegistered().get("com.xdubbo.config.spring.api.DemoService");
            assertNotNull(urls2);
            assertEquals(1, urls2.size());
            assertEquals("dubbo://" + NetUtils.getLocalHost() + ":20880/com.xdubbo.config.spring.api.DemoService", urls2.get(0).toIdentityString());
        } finally {
            ctx.stop();
            ctx.close();
            exporter1.unexport();
    ...
    public void testDelayFixedTime() throws Exception {
        ...
        try {
            ...
            while ((i++) < 60 && urls == null) {
                urls = registryService.getRegistered().get("com.xdubbo.config.spring.api.DemoService");
                Thread.sleep(10);
            }
            assertNotNull(urls);
            assertEquals(1, urls.size());
            assertEquals("dubbo://" + NetUtils.getLocalHost() + ":20883/com.xdubbo.config.spring.api.DemoService", urls.get(0).toIdentityString());
        } finally {
            ctx.stop();
            ctx.close();
            exporter.unexport();
