 ...
 */
package com.xdubbo.remoting.buffer;
...
import static com.xdubbo.remoting.buffer.ChannelBuffers.wrappedBuffer;
...
public abstract class AbstractChannelBufferTest {
    ...
    public void testSequentialByteBufferBackedHeapBufferTransfer1() {
        byte[] valueContent = new byte[BLOCK_SIZE * 2];
        ChannelBuffer value = ChannelBuffers.wrappedBuffer(ByteBuffer.allocate(BLOCK_SIZE * 2));
        value.writerIndex(0);
        buffer.writerIndex(0);
        for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
            random.nextBytes(valueContent);
